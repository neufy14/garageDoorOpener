...
class adjustCamera:
    def __init__(self):
        ...
        self.jsonFilename = "cameraData.json"
        self.deviceLoc = geocoder.ip('me')
        adjustCamera.isDaytime = True

    def checkModeVsTime(self):
        self.cameraLogger.debug("check mode verse time")
        while True:
            data = self.parseJson(self.jsonFilename)
            mode = data["currentMode"]
            dayNight = data["dayNight"]
            if mode == "auto":
                # self.cameraLogger.debug("Mode is auto")
                sunRise, sunSet = self.getSunHours(self.deviceLoc.latlng)
                ...
                if currentTime > sunRise and currentTime < sunSet:
                    ...
                    if dayNight == "night":
                        ...
                        # adjustCamera.settingsChange = False
                        data["dayNight"] = "day"
                        self.saveJson(self.jsonFilename, data)
                    # print("day time")
                else:
    ...
    def switchMode(self, mode):
        # adjustCamera.settingsChange = True
        self.cameraLogger.debug("very start of switch")
        try:
            display = Display(visible=0, size=(800, 800))  
            display.start()
        except Exception as e:
            self.cameraLogger.debug(f"An error occurred: {e}")
        else:
            ...
            self.cameraLogger.debug("Done with camera switching")
